...
export default function StoreBirthdayPage({ store }: { store: Store }) {
  ...
  const resetOne = (id: string) => updateMember(id, { whatsappSent: false })
  const resetAll = () =>
    members
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="interactive-card rounded-2xl border border-silver-700 bg-white_smoke-900 p-4 shadow-sm shadow-silver-400/30 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="grid w-full grid-cols-[1.2fr_1fr_1fr_0.8fr_auto] gap-3 rounded-lg bg-white_smoke-800 px-4 py-3 text-xs font-semibold uppercase tracking-[0.1em] text-silver-700">
            <span>Nama</span>
            <span>No WA</span>
        ...
        <div className="divide-y divide-silver-700">
          {list.map((m) => {
            ...
            return (
              <div key={m.id} className="grid grid-cols-[1.2fr_1fr_1fr_0.8fr_auto] items-center gap-3 px-4 py-3 text-sm text-carbon_black-500">
