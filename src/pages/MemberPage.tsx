...
function AllMembersTable({ members, onDelete }: { members: Member[]; onDelete: (id: string) => void }) {
  return (
    <div className="mt-6 rounded-2xl border border-silver-700 bg-white_smoke-900 p-4 shadow-md shadow-silver-400/30">
      ...
      <div className="mt-3 divide-y divide-silver-700 text-sm">
        {members.map((m) => (
          <div key={m.id} className="grid grid-cols-[1.2fr_1fr_1fr_1fr_auto] items-center gap-3 py-3">
            <div>
            ...
            <span className={m.status === 'active' ? 'text-strawberry_red-500' : 'text-silver-600'}>{m.status}</span>
      ...
      <section className="rounded-2xl border border-silver-700 bg-white_smoke-900 p-5 shadow-md shadow-silver-400/30">
        ...
        <div className="mt-4 space-y-2">
          ...
          {pendingMembers.map((m) => (
            <PendingRow key={m.id} member={m} onApprove={() => approvePending(m.id)} onReject={() => setConfirmId(m.id)} />
          ))}
          {pendingMembers.length === 0 ? <p className="px-4 py-6 text-sm text-silver-600">Tidak ada data pending.</p> : null}
        </div>
      </section>

      <AllMembersTable members={members} onDelete={deleteMember} />
